...
Page({
  ...
  onLoad(){
    let warehouse = app.globalData.warehouseData.warehouse;
    let units     = app.globalData.warehouseData.units;

    let unitArray = new Array();
    for(let i = 0 ;i<units.length;i++){
      let unit = units[i];
      let floorArray = ['一楼', '二楼', '三楼', '四楼', '五楼', '六楼'];
      let floor = 0;
      for(let loop = 0;loop<6;loop++){
        if(floorArray[loop] == unit.floor)
          floor = loop;
      }
      let could = true;
      let couldStr = "可分租"
      if (unit.couldSublet==0) {could = false;couldStr='不可分租'}

      let supportings = unit.supportingFacilities;
      ...
      let obj = {
        ...
        area: unit.area,
        couldSublet: '可分租',
        minSubletArea: unit.minSubletArea,
        isminSublet:could,
        price: unit.price,
        facilitys: [
          { uid: i, name: "月台", value: '0', checked: supportings.indexOf("月台") > -1 ? true : false },
          { uid: i, name: "喷淋", value: '1', checked: supportings.indexOf("喷淋") > -1 ? true : false },
          { uid: i, name: "行车", value: '2', checked: supportings.indexOf("行车") > -1 ? true : false },
        ],
      }
    ...
    this.setData({
      region: warehouse.locate.split(" "),
      parkService: warehouse.parkService.replace(/ /g,","),
      remark: warehouse.remark,
      suitableFor: warehouse.suitableFor.replace(/ /g, ","),
      advantage: warehouse.advantage.replace(/ /g,","),
      unitArray: unitArray,
    })
  ...
  formSubmit: function (e) {
    ...
    let openId = app.globalData.warehouseData.openId;
    ...
    for (let j = 0; j < userUnits.length ;j++){
      ...
      let unitTemp = {
        warehouseId : app.globalData.warehouseData.warehouse.wid,
        uid: userUnits[j].uid,
    ...
    console.log(app.globalData.warehouseData.warehouse)


    wx.request({
      method: 'POST',
      url: app.globalData.url + '/api/warehouse/update-warehouse', //接口地址
      data: {
        wid: app.globalData.warehouseData.warehouse.wid,
        isPass: 1,
        openId: app.globalData.warehouseData.warehouse.openId,
        advantage: advantage,
        createTime: util.formatDate(new Date()),
        status: app.globalData.warehouseData.warehouse.status,
        locate: city,
        parkService: parkService,
        picture: picStr,
        remark: remark,
    ...
    let unitStr = JSON.stringify(units);
    wx.request({
      method: 'POST',
      url: app.globalData.url + '/api/warehouse/update-unit', //接口地址
      data: unitStr
      ,
      success: function (res) {
